  ...
  Object.assign(options, {
    ...
    ready: () => {
      cache.on(
        ...
        listenOnAuthorizationReturnUri,
        true
      );
      ...
      cache.on(/\.reorderThisList$/, listenOnReorderPlaylist);
      ...
      cache.on(
        "player.shuffle",
        listenOnShuffle,
        adapter.config.defaultShuffle || "on"
      );
      ...
      cache.on(
        ["player.playlist.trackList", "player.playlist.trackListArray"],
...
function main() {
  ...
  if (isEmpty(application.statusPollingDelaySeconds)) {
    ...
    application.statusPollingDelaySeconds
  ) {
    application.statusPollingDelaySeconds = 1;
  }
...
function readTokenStates() {
  ...
  if (state) {
    ...
    let validRefreshToken = !isEmpty(
      loadOrDefault(tokenObj, "refreshToken", "")
    );
    let validClientId =
      !isEmpty(loadOrDefault(tokenObj, "clientId", "")) &&
      tokenObj.clientId === application.clientId;
    let validClientSecret =
      !isEmpty(loadOrDefault(tokenObj, "clientSecret", "")) &&
    ...
    switch (response.statusCode) {
        ...
        if (parsedBody.error.message === "The access token expired") {
          ...
          ret = Promise.all([
            ...
            .catch((err) => {
              if (err === 202) {
                adapter.log.debug(
                  err + " request accepted but no data, try again"
                );
              } else {
      ...
      case 429:
        ...
        ret = new Promise((resolve) =>
          setTimeout(() => !stopped && resolve(), wait * 1000)
    ...
    .then(() => {
      ...
      let fn = function (artist) {
        if (artistImageUrlCache.hasOwnProperty(artist)) {
          ...
          return sendRequest("/v1/artists/" + artist, "GET", "").then(
            (parseJson) => {
    ...
    .then(() => {
      ...
      if (type === "playlist" && uri) {
        ...
        return cache.setValue("player.playlist.id", playlistId).then(() => {
          let refreshPlaylist = function (parseJson) {
            ...
            return Promise.all([
              ...
              .then(() => {
                if (
                  cache.getValue(`playlists.${prefix}.trackListIds`) === null
                ) {
              ...
              .then(() => {
                const promises = [
                  copyState(
                    `playlists.${prefix}.trackListNumber`,
                    "player.playlist.trackListNumber"
                  ),
                  copyState(
                    `playlists.${prefix}.trackListString`,
                    "player.playlist.trackListString"
                  ),
                  copyState(
                    `playlists.${prefix}.trackListStates`,
                  ...
                  copyState(
                    `playlists.${prefix}.trackListIdMap`,
                    "player.playlist.trackListIdMap"
                  ),
                  copyState(
                    `playlists.${prefix}.trackListIds`,
                    "player.playlist.trackListIds"
                  ),
                  copyState(
                    `playlists.${prefix}.trackListArray`,
          ...
          if (playlistCache.hasOwnProperty(userId + "-" + playlistId)) {
            ...
            return sendRequest(
              `/v1/users/${userId}/playlists/${playlistId}?${querystring.stringify(
                query
              )}`,
              "GET",
              ""
        ...
        return Promise.all([
          ...
          Promise.all([listenOnHtmlPlaylists(), listenOnHtmlTracklist()])
        );
      }
    })
    .then(() =>
...
function createPlaylists(parseJson, autoContinue, addedList) {
  ...
  let fn = function (item) {
    ...
    if (isEmpty(playlistName)) {
      adapter.log.warn("empty playlist name " + JSON.stringify(item));
    ...
    return Promise.all([
      ...
      .then((playlistObject) => {
        ...
        let currentPlaylistOwnerId = cache.getValue(
        ...
        if (
          `${ownerId}-${playlistId}` ===
          `${currentPlaylistOwnerId}-${currentPlaylistId}`
        ) {
  ...
  for (let i = 0; i < parseJson.items.length; i++) {
    p = p
      .then(
        () =>
          new Promise((resolve) =>
            setTimeout(() => !stopped && resolve(), 1000)
          )
      )
...
function getUsersPlaylist(offset, addedList) {
  ...
  if (!isEmpty(application.userId)) {
    ...
    return sendRequest(
      `/v1/users/${application.userId}/playlists?${querystring.stringify(
        query
      )}`,
      "GET",
      ""
  ...
  while (true) {
    ...
    try {
      ...
      data.items.forEach((item) => {
        ...
        playlistObject.stateString += `${no}:${cleanState(
          trackName
        )} - ${cleanState(artist)}`;
        playlistObject.listString += `${cleanState(trackName)} - ${cleanState(
          artist
        )}`;
      ...
      adapter.log.warn(
        "error on load tracks(getPlaylistTracks): " +
          err +
          " owner: " +
          owner +
          " id: " +
...
function saveToken(data) {
  ...
  if (
    "undefined" !== typeof data.access_token &&
    ...
    .catch((err) => {
      ...
      if (
        ...
        if (err === 202) {
          ...
          adapter.log.warn(
            "unexpected api response http " + err + "; continue polling"
          );
        }
  ...
  return r
    .then(() => {
      ...
      return sendRequest(
        ...
        .catch((err) =>
          adapter.log.error(
            `could not start playlist ${playlist} of user ${owner}; error: ${err}`
          )
        );
    })
    .then(() => {
...
function listenOnAuthorizationReturnUri(obj) {
  ...
  let returnUri = querystring.parse(
...
function listenOnGetAuthorization() {
  ...
  let options = {
    url:
      "https://accounts.spotify.com/de/authorize/?" +
...
function listenOnUseForPlayback(obj) {
  const lastDeviceId = cache.getValue(
    obj.id.slice(0, obj.id.lastIndexOf(".")) + ".id"
  );
  if (!lastDeviceId) {
    return;
  }
...
function listenOnReorderPlaylist(obj) {
  // Reorder Playlist
  const idState = cache.getValue(
    obj.id.slice(0, obj.id.lastIndexOf(".")) + ".id"
  );
...
function listenOnPause() {
  ...
  clearTimeout(application.statusInternalTimer);
  sendRequest(
    "/v1/me/player/pause?" + querystring.stringify(query),
    "PUT",
    "",
    true
  )
...
function listenOnSkipPlus() {
  ...
  clearTimeout(application.statusInternalTimer);
  sendRequest(
    "/v1/me/player/next?" + querystring.stringify(query),
    "POST",
    "",
    true
  )
...
function listenOnSkipMinus() {
  ...
  clearTimeout(application.statusInternalTimer);
  sendRequest(
    "/v1/me/player/previous?" + querystring.stringify(query),
    "POST",
    "",
    true
  )
...
function listenOnVolume(obj) {
  ...
  if (is_play && is_play.val) {
    clearTimeout(application.statusInternalTimer);
    sendRequest(
      "/v1/me/player/volume?volume_percent=" + obj.state.val,
      "PUT",
      "",
      true
    )
...
function listenOnGetDevices() {
  return sendRequest("/v1/me/player/devices", "GET", "").then((data) =>
    reloadDevices(data)
  );
}
...
function clearCache() {
  artistImageUrlCache = {};
  playlistCache = {};
  application.cacheClearHandle = setTimeout(
    () => !stopped && clearCache(),
    1000 * 60 * 60 * 24
  );
}
...
function listenOnHtmlPlaylists() {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    html +=
      "<td" +
      style +
      ' class="spotifyPlaylistsCol spotifyPlaylistsColTitle' +
      cssClassTitle +
      '">';
    html += strings[i];
    html += "</td>";
    html +=
      '<td class="spotifyPlaylistsCol spotifyPlaylistsColIcon' +
      cssClassIcon +
      '">';
    if (current === ids[i]) {
      html +=
...
function listenOnHtmlDevices() {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (current === ids[i]) {
      html += `<img style="width: 16px; height: 16px" class="spotifyDevicesIconActive" src="widgets/spotify-premium/img/${type
        .replace("black", "green")
        .replace("icons/", "")}" />`;
    } else {
      html += `<img style="width: 16px; height: 16px" class="spotifyDevicesIcon" src="widgets/spotify-premium/img/${type.replace(
        "icons/",
        ""
      )}" />`;
    }
